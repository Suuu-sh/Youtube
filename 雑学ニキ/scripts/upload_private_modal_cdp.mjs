...
async function main() {
  ...
  const target = pages.find((p) => p.type === 'page' && p.url.includes('studio.youtube.com/channel/UCwA8z3WDB34cbbLKlicktxg/videos/upload'))
    || pages.find((p) => p.type === 'page' && p.url.includes('studio.youtube.com/channel/UCwA8z3WDB34cbbLKlicktxg'))
  ...
  try {
    ...
    await waitFor(
      client,
      'existing upload dialog file input',
      async () =>
        await evalValue(
          client,
          `(() => document.querySelector('input[type="file"]') ? 'upload' : '')()`
        ),
      60000
    );
