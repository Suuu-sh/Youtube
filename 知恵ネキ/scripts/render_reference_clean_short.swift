...
func makeFrame(manifest: Manifest, images: [NSImage], durations: [Double], frame: Int, audioSeconds: Double) -> CGImage? {
    ...
    let z = CGFloat(1.10 + 0.025 * smooth(local))
    let panX = CGFloat((Double(sceneIndex % 3) - 1.0) * 16.0 * smooth(local))
    let panY = CGFloat(-20.0 + (Double((sceneIndex + 1) % 3) - 1.0) * 8.0 * smooth(local))
    ...
    drawText(manifest.badge, in: NSRect(x: 58, y: canvas.height - 228, width: 500, height: 34), font: .systemFont(ofSize: 29, weight: .heavy), color: gold)
    ...
    drawText(scene.tag, in: NSRect(x: 64, y: canvas.height - 324, width: canvas.width - 128, height: 42), font: .systemFont(ofSize: 32, weight: .heavy), color: gold)
    drawText(scene.title, in: NSRect(x: 62, y: canvas.height - 382, width: canvas.width - 124, height: 62), font: .systemFont(ofSize: 48, weight: .heavy), color: .white)

    let appear = smooth(min(local / 0.18, 1.0))
    let bounce = CGFloat(1.0 + 0.035 * sin(appear * .pi))
    let subBase = NSRect(x: 58, y: canvas.height * 0.415, width: canvas.width - 116, height: 270)
    let subRect = NSRect(
    ...
    if highlights.isEmpty {
        drawOutlinedText(scene.subtitle, in: subRect, font: .systemFont(ofSize: 65, weight: .black), color: .white, alignment: .center, lineSpacing: 11)
    } else {
        drawOutlinedText(scene.subtitle, in: subRect, font: .systemFont(ofSize: 65, weight: .black), color: .white, highlightWords: highlights, alignment: .center, lineSpacing: 11)
    }
