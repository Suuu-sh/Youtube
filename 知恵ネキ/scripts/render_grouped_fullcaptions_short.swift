...
func makeFrame(manifest: Manifest, images: [NSImage], durations: [Double], frame: Int, audioSeconds: Double) -> CGImage? {
    ...
    let subBase = NSRect(x: 72, y: 720, width: canvas.width - 144, height: 430)
    let subRect = NSRect(
    ...
    if sceneIndex == 0 {
        ...
        drawOutlinedText(scene.subtitle, in: subRect, font: .systemFont(ofSize: 47, weight: .black), color: .white, alignment: .center, lineSpacing: 9)
    } else {
        drawOutlinedText(scene.subtitle, in: subRect, font: .systemFont(ofSize: 47, weight: .black), color: .white, highlightWords: highlights, alignment: .center, lineSpacing: 9)
    }
